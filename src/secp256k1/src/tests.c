...
void program_main(const argdata_t *ad) {
    ...
    if (cloudabi_sys_random_get(seed16, sizeof(seed16)) != 0) {
    ...
    while (argdata_map_next(&it, &key, &value)) {
        ...
        if (argdata_get_str_c(key, &keystr) == 0) {
            if (strcmp(keystr, "count") == 0) {
                argdata_get_int(value, &count);
            } else if (strcmp(keystr, "seed") == 0) {
                const char *ch;
                if (argdata_get_str_c(value, &ch) == 0) {
                    int pos = 0;
            ...
            } else if (strcmp(keystr, "terminal") == 0) {
                int fd = -1;
                FILE *fp;
    ...
    fprintf(stderr, "test count = %i\n", count);
    fprintf(stderr, "random seed = %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", seed16[0], seed16[1], seed16[2], seed16[3], seed16[4], seed16[5], seed16[6], seed16[7], seed16[8], seed16[9], seed16[10], seed16[11], seed16[12], seed16[13], seed16[14], seed16[15]);
    ...
    secp256k1_rand256(run32);
    fprintf(stderr, "random run = %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n", run32[0], run32[1], run32[2], run32[3], run32[4], run32[5], run32[6], run32[7], run32[8], run32[9], run32[10], run32[11], run32[12], run32[13], run32[14], run32[15]);
